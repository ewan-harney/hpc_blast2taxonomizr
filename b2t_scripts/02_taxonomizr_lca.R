...
blastResults<-read.table(file = paste(path, "/blast_out/filtered_blast.out.tab", sep=""),header=FALSE,stringsAsFactors=FALSE)
...
# Write output
write.table(taxon_path, "blast_out/taxonomizr_taxon_path.tsv", sep="\t", quote = FALSE)

# Email user
email_plot_command <- paste("echo \"Taxonomizr is complete.\" | mail -s \"Taxonomizr table\" -a blast_out/taxonomizr_taxon_path.tsv", opt$email, sep=" ")
